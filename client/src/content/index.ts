...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message.type === "CONTENT_START_TRACKING") {
    // Reset background state first, then start fresh
    chrome.runtime
      .sendMessage({ type: "RESET_SESSION" })
      .then(() => keystrokeTracker.start())
  ...
  if (message.type === "CONTENT_RESET") {
    // Full reset - stop tracking and clear background state
    keystrokeTracker
      .stop()
      .then(() => chrome.runtime.sendMessage({ type: "RESET_SESSION" }))
      .then(() => sendResponse({ success: true }))
      .catch((err) => sendResponse({ success: false, error: err.message }));
    return true;
  }
  ...
  if (message.type === "CONTENT_GET_STATUS") {
    ...
    sendResponse({ success: true, status: keystrokeTracker.status });
    return true;
  }
...
function captureText(): string {
  ...
  if (proseMirror) {
    ...
    if (text.length > 0) {
      console.log(
        "[HumanSign] Captured text from ProseMirror:",
        text.length,
        "chars",
      );
      return text;
  ...
  if (monacoLines.length > 0) {
    ...
    if (text.length > 0) {
      console.log(
        "[HumanSign] Captured text from Monaco:",
        text.length,
        "chars",
      );
      return text;
  ...
  if (active) {
    if (
      ...
      if (active.value.length > 0) {
        console.log(
          "[HumanSign] Captured text from input/textarea:",
          active.value.length,
          "chars",
        );
        return active.value;
    ...
    if (
      ...
      if (text.length > 0) {
        console.log(
          "[HumanSign] Captured text from contenteditable:",
          text.length,
          "chars",
        );
        return text;
  ...
  if (selection && selection.length > 0) {
    console.log(
      "[HumanSign] Captured text from selection:",
      selection.length,
      "chars",
    );
    return selection;
    ...
    if (el) {
      if (el instanceof HTMLIFrameElement) {
        try {
          ...
          if (text.length > 0) {
            console.log(
              "[HumanSign] Captured text from iframe:",
              text.length,
              "chars",
            );
            return text;
      ...
      const text =
        el.textContent?.trim() || (el as HTMLTextAreaElement).value || "";
      if (text.length > 0) {
        console.log(
          "[HumanSign] Captured text from",
          selector,
          ":",
          text.length,
          "chars",
...
// Check if we should auto-start (if there's an active recording session)
chrome.runtime
  .sendMessage({ type: "GET_STATS" })
  .then((response: { stats?: { isRecording: boolean } }) => {
    if (response?.stats?.isRecording) {
      console.log(
        "[HumanSign] Active recording detected, auto-starting tracker",
      );
      keystrokeTracker.start().catch(console.error);
    }
  })
  .catch(() => {
