...
import {
    ...
    Undo, Redo, Quote, Code, Sparkles, FileDown, Printer, PaintRoller,
    Minus, Plus, ChevronDown, Highlighter, Link2, MessageSquarePlus, Image,
    IndentDecrease, IndentIncrease, Star, Cloud, MessageSquare, Video, Lock, Menu,
    FilePlus, Download, ExternalLink, Printer as PrinterIcon
} from 'lucide-react';
...
export function DocumentEditor() {
    ...
    useState(() => {
        // We use a custom debounce via useEffect on contextText
    });
    ...
    return (
        <div className="flex flex-col h-screen bg-[#F9FBFD] w-full overflow-hidden font-sans text-sm" onKeyDown={handleKeyDown}>
                ...
                <div className="flex flex-col gap-0.5 w-full">
                    ...
                    <div className="flex items-center gap-1 text-[13px] text-slate-600 select-none">
                        ...
                        {['Edit', 'View', 'Insert', 'Format', 'Tools', 'Extensions', 'Help'].map((item) => (
                            <button key={item} className="px-2 py-0.5 rounded hover:bg-slate-100 transition-colors">
                                {item}
                            </button>
                        ))}
                    </div>
            ...
            <div className="w-full px-4 py-1 bg-[#EDF2FA] border-b border-gray-300 flex items-center gap-1.5 overflow-x-auto shrink-0 z-20">
                <div className="flex items-center gap-0.5 shrink-0">
                    <ToolbarBtn icon={Undo} onClick={() => editor.chain().focus().undo().run()} disabled={!editor.can().undo()} tooltip="Undo" />
                    <ToolbarBtn icon={Redo} onClick={() => editor.chain().focus().redo().run()} disabled={!editor.can().redo()} tooltip="Redo" />
                    <ToolbarBtn icon={Printer} onClick={() => window.print()} tooltip="Print" />
                    <ToolbarBtn icon={PaintRoller} onClick={() => handleNotImplemented('Paint format')} tooltip="Paint format" />
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <button className="flex items-center gap-1 px-1 hover:bg-slate-200 rounded cursor-pointer h-7 border border-transparent hover:border-slate-300 focus:outline-none transition-colors ml-1">
                                <span className="text-slate-600 font-medium min-w-[3ch] text-right">{zoom}%</span>
                ...
                <div className="flex items-center gap-0.5 shrink-0">
                    ...
                    <div className="flex items-center gap-1">
                        <ToolbarBtn icon={Minus} size="xs" onClick={() => handleNotImplemented('Decrease Font')} />
                        <div className="w-8 text-center border px-1 h-6 flex items-center justify-center rounded bg-white text-slate-700 border-slate-300 hover:border-blue-500 focus-within:border-blue-500">11</div>
                        <ToolbarBtn icon={Plus} size="xs" onClick={() => handleNotImplemented('Increase Font')} />
                    </div>
                </div>
                ...
                <div className="flex items-center gap-0.5 shrink-0">
                    <ToolbarBtn icon={Bold} onClick={() => editor.chain().focus().toggleBold().run()} active={editor.isActive('bold')} tooltip="Bold" />
                    <ToolbarBtn icon={Italic} onClick={() => editor.chain().focus().toggleItalic().run()} active={editor.isActive('italic')} tooltip="Italic" />
                    <ToolbarBtn icon={UnderlineIcon} onClick={() => editor.chain().focus().toggleUnderline().run()} active={editor.isActive('underline')} tooltip="Underline" />
                    <div className="flex flex-col items-center justify-center h-7 w-8 hover:bg-slate-200 rounded cursor-pointer relative group">
                        <span className="font-bold text-slate-700 border-b-4 border-black pb-0.5 leading-none px-1">A</span>
                    </div>
                    <ToolbarBtn icon={Highlighter} onClick={() => handleNotImplemented('Highlight')} tooltip="Highlight color" />
                </div>

                <Divider />

                <div className="flex items-center gap-0.5 shrink-0">
                    <ToolbarBtn icon={Link2} onClick={() => handleNotImplemented('Insert Link')} tooltip="Insert link" />
                    <ToolbarBtn icon={MessageSquarePlus} onClick={() => handleNotImplemented('Add Comment')} tooltip="Add comment" />
                    <ToolbarBtn icon={Image} onClick={() => handleNotImplemented('Insert Image')} tooltip="Insert image" />
                </div>

                <Divider />

                <div className="flex items-center gap-0.5 shrink-0">
                    <ToolbarBtn icon={AlignLeft} onClick={() => editor.chain().focus().setTextAlign('left').run()} active={editor.isActive({ textAlign: 'left' })} tooltip="Left align" />
                    <ToolbarBtn icon={AlignCenter} onClick={() => editor.chain().focus().setTextAlign('center').run()} active={editor.isActive({ textAlign: 'center' })} tooltip="Center align" />
                    <ToolbarBtn icon={AlignRight} onClick={() => editor.chain().focus().setTextAlign('right').run()} active={editor.isActive({ textAlign: 'right' })} tooltip="Right align" />
                    <ToolbarBtn icon={AlignJustify} onClick={() => editor.chain().focus().setTextAlign('justify').run()} active={editor.isActive({ textAlign: 'justify' })} tooltip="Justify" />
                </div>

                <Divider />

                <div className="flex items-center gap-0.5 shrink-0">
                    <ToolbarBtn icon={ListOrdered} onClick={() => editor.chain().focus().toggleOrderedList().run()} active={editor.isActive('orderedList')} tooltip="Checklist" />
                    <ToolbarBtn icon={List} onClick={() => editor.chain().focus().toggleBulletList().run()} active={editor.isActive('bulletList')} tooltip="Bullet list" />
                    <ToolbarBtn icon={IndentDecrease} onClick={() => handleNotImplemented('Decrease Indent')} tooltip="Decrease indent" />
                    <ToolbarBtn icon={IndentIncrease} onClick={() => handleNotImplemented('Increase Indent')} tooltip="Increase indent" />
                </div>
                ...
                <div className="ml-auto flex items-center pl-2 border-l border-slate-300">
                    <div className="flex items-center gap-2 text-xs text-slate-500 px-3 cursor-default select-none">
                        <div className={cn("h-2 w-2 rounded-full", wordCount > 0 ? "bg-green-500 animate-pulse" : "bg-slate-300")} />
            ...
            <div className="flex-1 flex overflow-hidden relative">
                        ...
                        {headings.length > 0 ? headings.map((heading, i) => (
                            <button
                                ...
                                className={cn(
                                    ...
                                    heading.level === 1 ? "font-semibold text-slate-700" : "text-slate-500 pl-2",
                            ...
                            <div className="text-xs text-slate-400 font-medium px-2">Headings will appear here.</div>
                        )}
                    </div>
                </div>
                    ...
                    <div style={{ zoom: zoom / 100 }}>
                        ...
                        <div className="w-[816px] h-6 bg-white border-b flex items-end px-[96px] relative mb-4 select-none mx-auto">
                            {Array.from({ length: 8 }).map((_, i) => (
                                <div key={i} className="flex-1 border-l border-slate-300 h-1.5 relative group">
                                    <span className="absolute -top-3 -left-1 text-[9px] text-slate-500 opacity-0 group-hover:opacity-100">{i + 1}</span>
                                </div>
                            ))}
                        ...
                        <div className="bg-white w-[816px] min-h-[1056px] shadow-[0_2px_8px_rgba(0,0,0,0.1)] border border-slate-200 px-[96px] py-[96px] cursor-text print:shadow-none print:border-none print:w-full mx-auto">
                            <EditorContent editor={editor} className="outline-none h-full [&_.ProseMirror]:min-h-[900px] [&_.ProseMirror]:outline-none text-[11pt] leading-[1.6] [&_p]:mb-4" />
                        </div>
                    </div>
                </div>
                ...
                <div className="hidden 2xl:flex flex-col items-center gap-6 w-14 pt-4 border-l bg-white h-full shrink-0 z-10">
                    <div className="h-10 w-10 rounded-full hover:bg-slate-100 flex items-center justify-center cursor-pointer transition-colors" title="Calendar">
                        <img src="https://www.gstatic.com/companion/icon_assets/calendar_2020q4_2x.png" className="w-5 h-5" alt="Calendar" />
                    </div>
                    <div className="h-10 w-10 rounded-full hover:bg-slate-100 flex items-center justify-center cursor-pointer transition-colors" title="Keep">
                        <img src="https://www.gstatic.com/companion/icon_assets/keep_2020q4v3_2x.png" className="w-5 h-5" alt="Keep" />
                    </div>
                    <div className="h-10 w-10 rounded-full hover:bg-slate-100 flex items-center justify-center cursor-pointer transition-colors" title="Tasks">
                        <img src="https://www.gstatic.com/companion/icon_assets/tasks_2021_2x.png" className="w-5 h-5" alt="Tasks" />
                    </div>
                    <div className="h-10 w-10 rounded-full hover:bg-slate-100 flex items-center justify-center cursor-pointer transition-colors" title="Contacts">
                        <img src="https://www.gstatic.com/companion/icon_assets/contacts_2022_2x.png" className="w-5 h-5" alt="Contacts" />
                    </div>
                ...
                {suggestion && (
                    <div className="absolute bottom-8 right-8 bg-white/90 backdrop-blur border border-purple-200 shadow-lg p-4 rounded-xl max-w-sm animate-in fade-in slide-in-from-bottom-2 z-50 ring-1 ring-purple-100">
                        <div className="flex items-center gap-2 text-purple-600 font-semibold text-xs mb-1.5 uppercase tracking-wide">
                            ...
                            <span className="ml-auto text-[10px] bg-purple-100 px-1.5 py-0.5 rounded text-purple-700">Tab to accept</span>
                        </div>
...
function ToolbarBtn({ icon: Icon, onClick, active, disabled, tooltip, size = 'sm' }: any) {
    return (
        <button
            onClick={onClick}
            disabled={disabled}
        ...
        >
            <Icon className={cn("text-slate-700", size === 'xs' ? "w-3 h-3" : "w-4 h-4", active && "text-[#0b57d0]")} />
        </button>
    );
}
